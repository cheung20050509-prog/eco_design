...
\author{
  Author Name \\
  Student ID: XXXXXXXX \\
  School of Economics, XXX University \\
...
% ============================================================
\begin{abstract}
Quantitative trading strategies powered by deep learning have gained significant traction in recent years, yet most existing approaches struggle to capture cross-asset correlations in multi-stock portfolios. This paper proposes a quantitative trading system for the Chinese A-share market based on the iTransformer model, a novel architecture from ICLR 2024 that applies inverted attention across the stock dimension rather than the temporal dimension. We construct a universe of 100 stocks spanning seven sectors from the CSI 300 index, covering the period from January 2018 to December 2025, and engineer 81 technical features across 13 categories. The iTransformer encoder learns cross-stock correlation patterns through inverted multi-head self-attention on a 100$\times$100 attention matrix, while a multi-scale temporal convolution module encodes each stock's historical time series into fixed-dimensional representations. The prediction target is the 5-day forward return, trained with a Huber loss combined with a directional auxiliary task. To ensure methodological rigor, we adopt a strict chronological train/validation/test split (70\%/15\%/15\%), fit the StandardScaler exclusively on training data to prevent data leakage, fix all random seeds for full reproducibility, and employ early stopping based on validation loss. The trading strategy integrates model predictions with a nine-rule signal filtering mechanism based on classical technical indicators and employs the Kelly criterion for dynamic position sizing. A two-phase Bayesian hyperparameter optimization framework using Optuna is designed to decouple model architecture tuning (Phase~1) from strategy parameter tuning (Phase~2), with Phase~2 adopting a ``train-once, sweep-strategy'' approach that evaluates 100 strategy configurations in minutes. Backtesting on the held-out test set yields a 13.47\% annualized return, 2.99\% maximum drawdown, and 1.254 Sharpe ratio---closely matching the validation-set performance (14.57\% return, 1.331 Sharpe), confirming that the val-based Optuna scoring does not overfit. These results validate the effectiveness of inverted attention for modeling inter-stock dependencies and highlight the importance of systematic hyperparameter optimization and strict data integrity in quantitative strategy design.
\end{abstract}

\textbf{Keywords:} iTransformer; Quantitative Trading; A-Share Market; Deep Learning; Kelly Criterion; Hyperparameter Optimization; Data Leakage Prevention
...
A complete quantitative trading system involves not only prediction models but also signal generation, risk management, and position sizing. Classical strategies include moving-average crossover, momentum strategies, and mean-reversion strategies \citep{murphy1999technical}. The Kelly criterion \citep{kelly1956new}, originally from information theory, provides a mathematically optimal framework for determining bet sizes that maximize the long-term growth rate of capital, given a known win probability and profit-loss ratio.
...
All market data used in this study are obtained from the Tushare financial data API (\texttt{tushare.pro}), a widely used data source for Chinese A-share market research. We select 100 stocks from the CSI 300 index, distributed across seven major sectors to ensure broad market coverage and sector diversity. Table~\ref{tab:stock_universe} summarizes the sector composition.
...
All 100 selected stocks have listing dates prior to January 2018, ensuring complete data coverage over the study period. The data spans from December 2018 to February 2025, providing approximately 1,315 common trading days across all stocks after date alignment (7+ years of individual data, with the common intersection starting from December 2018). For each stock, we collect daily OHLCV data (open, high, low, close, volume) along with turnover rate and basic financial information. Data are cached in a local SQLite database to enable reproducible experiments without repeated API calls.
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Missing Value Handling:} Stocks with extended suspension periods (e.g., IPO delays) may have missing entries. We apply forward-fill for gaps of up to 5 consecutive trading days and remove stocks with longer gaps from the affected windows. The resulting dataset retains all 100 stocks with complete records for the analysis period.
    ...
    \item \textbf{Data Alignment:} Since iTransformer requires synchronized data across all stocks (constructing $N \times T$ attention matrices), we align all data to a common trading calendar by taking the intersection of all stocks' available dates. To maximize the common date range, we replace 5 late-listing stocks (listed after 2018) with CSI~300 constituents that have data from at least January 2018, yielding 1,315 common trading days.

    \item \textbf{Train-Validation-Test Split:} The data are split chronologically into three non-overlapping sets: training (approximately 70\%), validation (approximately 15\%), and testing (approximately 15\%). After constructing sliding-window sequences of length $T_{\text{seq}} = 60$ and aligning all 100 stocks to a common trading calendar, this yields approximately 920 training days, 197 validation days, and 198 test days. This strict chronological split prevents any form of look-ahead bias.
...
The iTransformer model produces a predicted 5-day forward return $\hat{r}_i$ for each stock $i$ on each trading day. Raw trading signals are generated by comparing $\hat{r}_i$ against a buy threshold $\theta_b$ and a sell threshold $\theta_s$:

\begin{equation}
\text{Signal}_i =
\begin{cases}
\text{BUY}  & \text{if } \hat{r}_i > \theta_b \\
\text{SELL} & \text{if } \hat{r}_i < \theta_s \\
\text{HOLD} & \text{otherwise}
\end{cases}
\end{equation}

The default thresholds from the optimized configuration are $\theta_b = 2.387\%$ and $\theta_s = -0.432\%$.
...
\begin{itemize}[leftmargin=2em]
    \item Search over: $d_{\text{model}} \in [256, 2048]$, $d_{ff}$ ratio $\in \{2, 4, 8\}$, $n_{\text{heads}} \in \{4, 8, 16\}$, $n_{\text{layers}} \in [4, 16]$, $T_{\text{seq}} \in \{15, 30, 60\}$, dropout $\in [0.05, 0.4]$, learning rate $\in [10^{-5}, 10^{-3}]$, batch size $\in \{64, 128, 256, 512\}$, epochs $\in [100, 1000]$
    \item Strategy parameters fixed to best known values from prior experiments
    \item Optimization target: composite score $S = 0.5 \cdot \text{Sharpe} + 0.3 \cdot R_{\text{annual}} - 0.2 \cdot D_{\text{max}}$
    \item 20 trials with TPE sampler ($n_{\text{startup}} = 5$)
\end{itemize}

\textbf{Phase 2 --- Strategy Parameter Search (5 parameters):}
\begin{itemize}[leftmargin=2em]
    \item Search over: $\theta_b \in [0.005, 0.05]$, $\theta_s \in [-0.05, -0.005]$, stop-loss $\in [0.02, 0.10]$, take-profit $\in [0.05, 0.30]$, max position $\in [0.10, 0.50]$
    ...
    \item Same composite score objective
...
The composite optimization score balances three key metrics:
\begin{equation}
S = 0.5 \cdot \text{Sharpe} + 0.3 \cdot R_{\text{annual}} - 0.2 \cdot D_{\text{max}}
\end{equation}
where the Sharpe ratio \citep{sharpe1966mutual} is defined as:
\begin{equation}
\text{Sharpe} = \frac{\mathbb{E}[R_p - R_f]}{\sigma_p}
\end{equation}
with $R_p$ the portfolio return, $R_f$ the risk-free rate (set to 3\%), and $\sigma_p$ the portfolio volatility. This multi-objective formulation rewards high risk-adjusted returns while penalizing strategies with excessive drawdown.
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Buy-and-Hold Strategy:} An equal-weight portfolio of all 100 stocks is purchased at the beginning of the test period and held until the end. This represents the passive investment baseline.
...
Table~\ref{tab:optuna_best} summarizes the key results from the two-phase Optuna optimization. The best configuration from Phase~1 (model search) achieves significantly better performance than default parameters.

\begin{table}[H]
\centering
\caption{Best trial configuration from two-phase Optuna optimization (val-scored).}
\label{tab:optuna_best}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Parameter} & \textbf{Phase~1 Best} & \textbf{Phase~1 Runner-up} & \textbf{Phase~2 Best} \\
\midrule
$d_{\text{model}}$     & 1536   & 512    & 1536 (fixed) \\
$d_{ff}$               & 3072   & 2048   & 3072 (fixed) \\
$n_{\text{heads}}$     & 32     & 8      & 32 (fixed) \\
$n_{\text{layers}}$    & 14     & 16     & 14 (fixed) \\
$T_{\text{seq}}$       & 60     & 60     & 60 (fixed) \\
Dropout                & 0.207  & 0.127  & 0.207 (fixed) \\
Learning Rate          & $2.45 \times 10^{-4}$ & $1.25 \times 10^{-4}$ & (fixed) \\
Batch Size             & 64     & 512    & (fixed) \\
$\theta_b$ (buy)       & (fixed) & (fixed) & 0.02387 \\
$\theta_s$ (sell)      & (fixed) & (fixed) & $-$0.00432 \\
Stop-loss              & (fixed) & (fixed) & 5.88\% \\
Take-profit            & (fixed) & (fixed) & 8.21\% \\
Max position           & (fixed) & (fixed) & 34.50\% \\
\midrule
\multicolumn{4}{l}{\textit{Validation set performance (Optuna scoring basis):}} \\
Annual Return          & 7.88\%  & 7.80\%  & \textbf{14.57\%} \\
Max Drawdown           & 3.75\%  & 6.58\%  & \textbf{1.96\%} \\
Sharpe Ratio           & 0.690   & 0.432   & \textbf{1.331} \\
\midrule
\multicolumn{4}{l}{\textit{Test set performance (held-out, evaluated once):}} \\
\textbf{Annual Return} &  ---    &  ---    & \textbf{13.47\%} \\
\textbf{Max Drawdown}  &  ---    &  ---    & \textbf{2.99\%} \\
\textbf{Sharpe Ratio}  &  ---    &  ---    & \textbf{1.254} \\
Win Rate               &  ---    &  ---    & 45.83\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Validation--test consistency:} The validation-set annual return (14.57\%) and test-set annual return (13.47\%) differ by only 1.1 percentage points, and Sharpe ratios are similarly close (1.331 vs.\ 1.254). This narrow gap confirms that the val-based Optuna scoring does not overfit the strategy to in-sample data---a key methodological validation.
    \item \textbf{Model dimension:} The Phase~1 best ($d_{\text{model}} = 1536$, 14 layers) narrowly beats the runner-up ($d_{\text{model}} = 512$, 16 layers), suggesting that moderate model capacity with deeper layers is preferred when sufficient training data (920 days) is available.
    \item \textbf{Validation-based early stopping is critical:} Models stopped by validation loss generalize better than those stopped by training loss, as they avoid overfitting to training-set noise.
    \item \textbf{Phase 2 efficiency:} The ``train-once, sweep-strategy'' approach reduces Phase~2 from $\sim$67 hours (100 trials $\times$ 40 min) to $\sim$3 minutes (100 trials $\times$ 2 sec), enabling comprehensive strategy parameter exploration.
    \item \textbf{Direction accuracy:} The best model achieves a validation-set direction accuracy of 54.41\%, modestly above the 50\% random baseline. While small, this edge---when combined with optimized trading rules and position sizing---is sufficient to generate consistent positive returns.
\end{itemize}

\subsection{Strategy Comparison}

Table~\ref{tab:comparison} presents the comprehensive performance comparison between our iTransformer strategy and the two benchmark strategies on the test set (2025).

\begin{table}[H]
\centering
\caption{Performance of the iTransformer strategy on the held-out test set.}
\label{tab:comparison}
\begin{tabular}{lcccc}
\toprule
\textbf{Metric} & \textbf{Validation Set} & \textbf{Test Set (Held-out)} \\
\midrule
Annual Return (\%)  & 14.57 & \textbf{13.47} \\
Max Drawdown (\%)   & 1.96  & \textbf{2.99}  \\
Sharpe Ratio        & 1.331 & \textbf{1.254} \\
Win Rate (\%)       & 79.38 & 45.83 \\
Direction Accuracy  & ---   & 54.41\% \\
\bottomrule
\end{tabular}
\end{table}

The results demonstrate several notable characteristics:

\begin{enumerate}[leftmargin=2em]
    \item \textbf{Positive risk-adjusted returns:} The iTransformer strategy achieves a 13.47\% annualized return on the held-out test set with a Sharpe ratio of 1.254, indicating that each unit of risk generates meaningful excess return. A Sharpe ratio above 1.0 is generally considered strong for a systematic trading strategy.

    \item \textbf{Low drawdown:} The maximum drawdown of only 2.99\% reflects the conservative position sizing (max 34.5\% per stock) and effective stop-loss mechanism (5.88\%). This level of capital protection is attractive for risk-averse investors.

    \item \textbf{Validation--test generalization:} The close alignment between validation-set performance (14.57\% return, 1.331 Sharpe) and test-set performance (13.47\% return, 1.254 Sharpe) provides strong evidence that the results are not artifacts of overfitting. The Optuna scoring was based entirely on the validation set, and the test set was evaluated only once at the end.

    \item \textbf{Modest but real predictive power:} The direction accuracy of 54.41\% on the test set is above the 50\% random baseline and the 52\% historical base rate. While the edge is small in absolute terms, it is consistent with findings in the quantitative finance literature where single-factor ICs (information coefficients) of 0.02--0.05 are considered economically meaningful.
\end{enumerate}

\subsection{Visualization and Analysis}

Figure~\ref{fig:equity} shows the equity curve of the three strategies over the test period. The iTransformer strategy exhibits a steady upward trajectory with limited drawdowns, while the buy-and-hold strategy suffers from significant volatility, and the MA crossover strategy experiences frequent whipsaws.
...
Figure~\ref{fig:drawdown} visualizes the drawdown profile, clearly showing that the iTransformer strategy maintains drawdowns within the 10\% range, while the benchmarks frequently exceed 30--50\% drawdown.
...
While the optimized iTransformer strategy delivers strong performance, we note several important caveats:

\begin{itemize}[leftmargin=2em]
    \item \textbf{Modest directional edge:} The model achieves only $\sim$54\% direction accuracy, marginally above the 50\% random baseline. The profitability of the system relies not on strong prediction but on the coupling of a small but consistent edge with optimized position sizing and risk management. This is consistent with real-world systematic strategies, where single-factor ICs are typically 0.02--0.05.
...
Critically, the system enforces strict data integrity measures: a chronological 70\%/15\%/15\% train/validation/test split, StandardScaler fitted exclusively on training data to prevent data leakage, validation-based early stopping with best-model restoration, and fixed random seeds for reproducibility. These measures ensure that reported performance metrics reflect genuine out-of-sample generalization.

Through a two-phase Bayesian hyperparameter optimization framework using Optuna---with Phase~2 adopting a ``train-once, sweep-strategy'' approach that evaluates 100 strategy configurations in minutes---we successfully decouple the high-dimensional search space into model architecture search (9 parameters) and strategy parameter tuning (5 parameters). The optimized strategy achieves a 13.47\% annualized return with only 2.99\% maximum drawdown and a Sharpe ratio of 1.254 on the held-out test set. Crucially, the close alignment between validation-set performance (14.57\% return, 1.331 Sharpe) and test-set performance confirms that the val-based optimization does not overfit to in-sample data.
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Market relevance:} The system is designed specifically for the Chinese A-share market, with a diversified 100-stock universe from the CSI 300 index spanning seven sectors, ensuring broad applicability.
    
    \item \textbf{Methodological innovation:} The combination of iTransformer's inverted attention with traditional technical indicator-based signal filtering bridges the gap between modern deep learning and classical quantitative analysis, leveraging the strengths of both paradigms. The strict data integrity protocol (no leakage, validation-based early stopping, seed fixing) sets a high bar for reproducibility.
    
    \item \textbf{Rigorous evaluation:} The backtesting framework incorporates realistic transaction costs (commission, slippage, stamp tax), proper train/validation/test splitting, and two-benchmark comparison. All results are reproducible from the submitted code with fixed seeds.
    
    \item \textbf{Risk control:} The maximum drawdown of only 2.99\% on the test set demonstrates that the strategy maintains excellent capital protection through conservative position sizing (max 34.5\% per stock) and calibrated stop-loss (5.88\%), which is essential for practical deployment.
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Limited stock universe:} The strategy is evaluated only on 100 large-cap CSI 300 stocks. Its effectiveness on small-cap and mid-cap stocks, which have different liquidity and volatility characteristics, remains to be verified.
    
    \item \textbf{Single-period testing:} The test set covers only approximately 198 trading days. Different market regimes (e.g., prolonged bear markets, extreme volatility events) may yield different results. Multi-year rolling backtests would provide a more robust evaluation.
    ...
    \item \textbf{Hyperparameter sensitivity:} Performance is sensitive to hyperparameter choices, which underscores the importance of systematic optimization. The two-phase ``train-once, sweep-strategy'' approach partially mitigates this by enabling exhaustive strategy parameter search.
    
    \item \textbf{Strategy limited to daily frequency:} The current system operates on daily candles. Intraday strategies might capture additional alpha from shorter-term patterns, though this would require higher-frequency data and faster execution infrastructure.
\end{enumerate}
